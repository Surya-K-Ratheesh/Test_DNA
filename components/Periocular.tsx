...
type Profile = { name: string; embeddings: number[][]; skinRGB: number[] }
...
export default function Periocular() {
  ...
  async function handleRegister() {
    ...
    const newProfile: Profile = { name: name!, embeddings: captures, skinRGB: avgSkin }
          ...
          {profiles.map((p, i) => (
            <div key={i} className="flex items-center justify-between bg-gray-700 p-2 rounded">
              <div>
                ...
                <div className="text-xs text-gray-300">Skin RGB: {p.skinRGB.join(',')}</div>
              </div>
